...
class WarpPlugin(BasePlugin):
    ...
    def analyze_patterns(self, session_data: Dict) -> Dict[str, Any]:
        ...
        return {
            'command_frequency': dict(sorted(cmd_freq.items(), key=lambda x: x[1], reverse=True)[:20]),
            'project_activity': dict(sorted(project_activity.items(), key=lambda x: x[1], reverse=True)[:10]),
...
if __name__ == "__main__":
    ...
    parser.add_argument('command', choices=['test', 'recover', 'analyze'], 
                       help='Command to execute')
    parser.add_argument('--hours', type=int, default=24, 
                       help='Hours of history to process')
