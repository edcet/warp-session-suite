...
# Note: Skip editable install due to multiple top-level packages in flat layout
# Install dependencies directly instead
pip3 install "typer[all]>=0.9" "duckdb>=1.1.3" "rich>=13.7" || {
  echo "Failed to install Python dependencies"
  exit 1
}

echo "🛠️ Installing trunk CLI for code formatting and linting..."
curl -fsSL "$TRUNK_URL" | bash -s -- -b /usr/local/bin || {
  echo "Failed to install trunk CLI"
  exit 1
}

echo "🐍 Installing additional Python tools used by the format script..."
pip3 install black isort ruff PyYAML || {
  echo "Failed to install additional Python tools"
  exit 1
}
...
if ! command -v yq >/dev/null 2>&1; then
  ...
  chmod +x /usr/local/bin/yq || {
    echo "Failed to set executable permissions for yq"
    exit 1
  }
fi
...
if ! command -v shfmt >/dev/null 2>&1; then
  ...
  if [ -n "$SHFMT_URL" ]; then
    curl -fsSL "$SHFMT_URL" -o /usr/local/bin/shfmt || {
      echo "Failed to download shfmt"
      exit 1
    }
    chmod +x /usr/local/bin/shfmt || {
      echo "Failed to set executable permissions for shfmt"
      exit 1
    }
  else
